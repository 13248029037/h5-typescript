...
const Logone: React.SFC<IProps & RouteComponentProps> = (props) => {
  ...
  const pipelineHistoryLog = async (res) => {
    for (let container of res.stepName || []) {
      ...
      let body = logresult.body;
      if (body) {
        let result: ReadableStreamReadResult<any> = await body
          .getReader()
          .read();
        let log = (result.value || []).reduce((first, next) => {
          return first + String.fromCharCode(next);
        }, "");
        message = message + log;
      }
      setTime(Date.now());
  ...
  const onlineLogApp = async () => {
    ...
    const url = `https://devops-hub.tutorabc.com.cn/k8s/api/v1/namespaces/${getQueryString(
      ...
      "pop_name"
    )}/log?pretty=true&container=${getQueryString("pipeline_name")}`;
    let res = await fetchRequest(url, project_token);
    let body = res.body;
    if (body) {
      let result: ReadableStreamReadResult<any> = await body.getReader().read();
      let log = (result.value || []).reduce((first, next) => {
        return first + String.fromCharCode(next);
      }, "");
      setMsg(log);
